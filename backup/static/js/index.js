...
async function loadCameraConnectionStatus() {
    try {
        const response = await fetch(`/api/cameras/status?camera_id=${encodeURIComponent(selectedCamera)}`);
        const data = await response.json();
        console.log('摄像头状态API响应:', data);

        if (data.success) {
            // 更新连接状态显示
            updateConnectionStatus(data.is_connected, data.client_id);

            // 更新分辨率显示
            if (data.is_connected) {
                currentResolutionElement.textContent = `${data.width}x${data.height}`;
                currentFpsElement.textContent = data.fps.toFixed(1);
            } else {
                currentResolutionElement.textContent = '未连接';
                currentFpsElement.textContent = '0.0';
            }

            // 如果预览已启动但状态显示未连接，尝试重新连接
            if (isPreviewing && !data.is_connected) {
                console.log('检测到预览状态不一致，尝试重新连接...');
...
async function reconnectPreview() {
    ...
    try {
        ...
        console.error('重新连接预览失败:', error);
        showStatus('重新连接预览失败: ' + error.message, true);
    }
}

// 更新连接状态显示
function updateConnectionStatus(isConnected, clientId) {
    console.log('更新连接状态:', isConnected, clientId);
    ...
    if (isConnected) {
        ...
        // 更新分辨率
        currentResolutionElement.textContent = '已连接';
        currentFpsElement.textContent = '已连接';
    } else {
